...
public class Solution {
    ...
    public static void main(String[] args) throws NumberFormatException, IOException {
        ...
        for (int tc = 1; tc <= T; tc++) {
            ...
            // 주의: static 변수에 값을 할당해야 solve()에서 인식함 (int를 빼야 함)
        ...
        for (int time = 0; time < M; time++) {
            ...
            for (int i = 0; i < N; i++) {
                for (int j = 0; j < N; j++) {
                    ...
                    int cur_cnt = grid[i][j][0];
                    int cur_dir = grid[i][j][1];
                    int nr = i + dr[cur_dir];
                    int nc = j + dc[cur_dir];

                    // 1. 약품 구역(경계면)에 도착한 경우
                    if (nr == 0 || nc == 0 || nr == N - 1 || nc == N - 1) {
                        grid_tmp[nr][nc][0] = cur_cnt / 2;
                        // 방향 반전 로직
                        if (cur_dir == 1) grid_tmp[nr][nc][1] = 2;
                        else if (cur_dir == 2) grid_tmp[nr][nc][1] = 1;
                        else if (cur_dir == 3) grid_tmp[nr][nc][1] = 4;
                        else if (cur_dir == 4) grid_tmp[nr][nc][1] = 3;
                    } 
                    // 2. 일반 구역에 도착한 경우 (병합 로직 핵심)
                    else {
                        // 현재 이동해온 미생물 군집(cur_cnt)이 
                        // 이전에 이 칸에 들어온 어떤 군집보다도 크다면 방향 결정권 가짐
